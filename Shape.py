...
class Shape(object):
    def __init__(self):
        self.bones = [Bone(self._next_index()) for i in range(NUM_OF_BONES)]
    ...
    def _update_parts(self, parent, child, pointers):
        ...
        if len(self.parts[positionsters[parent]]) >= MAX_IN_PART:
...
class Spider(Shape):
    def __init__(self):
        ...
        for joint in body_joints:
            ...
            joint.max_angle = 35
            joint.min_angle = -35
            joint.action_factor = 0.1

        self.bones = body_bones
        self.joints = body_joints
        self.parts = [np.array(list(range(body_size)))]
    ...
    def _add_leg(self, parent_bone, hip_heading, leg_size = 4):
        ...
        foot = leg_bones[-1]
        foot.length, foot.height = foot.height, foot.length
        ankle = leg_joints[-1]
        ankle.max_angle = 20
        ankle.min_angle = -20
